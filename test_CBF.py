    ...
    def setupController(self, obs, dynamic_car_trajectories=None):
        ...
        for j in range(num_dynamic_cars):
            for i in range(self.N):
                ...
                self.opti.subject_to((dx ** 2 + dy ** 2) >= (0.8 + self.car.d) ** 2)  # Avoid collision

        # Add CBF constraints
        for i in range(self.N):
            h, Lfh, Lgh = self.cbf_constraint(self.X[i, :], self.U[i, :], obs)
            for hi, Lfhi, Lghi in zip(h, Lfh, Lgh):
                self.opti.subject_to(Lfhi + Lghi @ self.U[i, :] + 0.2 * hi >= 0)
        # dyna obj
        # Add CBF constraints for dynamic cars
        # if dynamic_car_trajectories is not None:
        #     for j in range(num_dynamic_cars):
        #         dynamic_obs = []
        #
        ...
        #         #for i in range(1):
        #             dynamic_obs.append(
        #                 [self.dynamic_cars_x[j][i], self.dynamic_cars_y[j][i], 0.8])  # Assuming radius 0.8
        #
        #         for i in range(self.N):
        #             h_dyn, Lfh_dyn, Lgh_dyn = self.cbf_constraint(self.X[i, :], self.U[i, :], dynamic_obs)
        #             for hi_dyn, Lfhi_dyn, Lghi_dyn in zip(h_dyn, Lfh_dyn, Lgh_dyn):
        #                 self.opti.subject_to(Lfhi_dyn + Lghi_dyn @ self.U[i, :] + 10 * hi_dyn >= 0)
...
def test_multi_car():
    ...
    car1 = Vehicle(2, 5, 0.0, 1.0, -0.0)
    car2 = Vehicle(8, 8, 0, -1.0, 0.0)
    ...
    with writer.saving(fig, "multi_car_animation.mp4", 600):
        while not mpc_controller.car.is_goal_reached(mpc_controller.car.path[-1], x_ref[-1]) or \
            ...
            if (distance([Xcar9.x, Xcar9.y], [car.x, car.y]) < 50):
                dynamic_car_trajectories1.append(Xcar9.prediction_traj(mpc_controller.N, u_ref_car2, mpc_controller.T))
            x_opt, u_opt = mpc_controller.planning(x_ref, u_ref, obs, dynamic_car_trajectories1)

            end_time1 = time.time()
            # Convert time to milliseconds
            execution_time_ms = (end_time1 - start_time) * 1000
            print(f"Time taken to compute factorial(10): {execution_time_ms:.6f} ms")
            a, omega = u_opt[0]
            # add opt traj
            ...
            if (distance([Xcar.x, Xcar.y], [car9.x, car9.y]) < 50):
                dynamic_car_trajectories2.append(Xcar.prediction_traj(mpc_controller.N, u_ref_car2, mpc_controller.T))
            ...
            end_time2 = time.time()
            # Convert time to milliseconds
            execution_time_ms = (end_time2 - end_time1) * 1000
            print(f"Time taken to compute factorial(10): {execution_time_ms:.6f} ms")
