...
from vehicle import Vehicle, copy
...
class MPCController:
    ...
    def control_barrier_function(self, x, obs):
        h = []
        for ob in obs:
            h.append((x[0] - ob[0]) ** 2 + (x[1] - ob[1]) ** 2 - (ob[2] + self.car.d) ** 2)
        return h

    def cbf_constraint(self, x, u, obs):
        h = self.control_barrier_function(x, obs)
        Lfh = []
        Lgh = []
        for hi, ob in zip(h, obs):
            Lfh.append(2 * (x[0] - ob[0]) * x[3] * ca.cos(x[2]) + 2 * (x[1] - ob[1]) * x[3] * ca.sin(x[2]))
            Lgh.append(2 * (x[0] - ob[0]) * ca.tan(x[4]) / self.car.L + 2 * (x[1] - ob[1]))
        return h, Lfh, Lgh
    ...
    def setupController(self, obs, dynamic_car_trajectories=None):
        ...
        for j in range(num_dynamic_cars):
            #self.opti.subject_to((state_x - self.dynamic_cars_x[j]) ** 2 + (state_y - self.dynamic_cars_y[j]) ** 2 >= (
            #            0.5 + self.car.d) ** 2)
            for i in range(self.N):
                ...
                self.opti.subject_to((dx ** 2 + dy ** 2) >= (0.2 + self.car.d) ** 2)  # Avoid collision

                # Add CBF constraints
        for i in range(self.N):
            h, Lfh, Lgh = self.cbf_constraint(self.X[i, :], self.U[i, :], obs)
            for hi, Lfhi, Lghi in zip(h, Lfh, Lgh):
                self.opti.subject_to(Lfhi + Lghi @ self.U[i, :] + 10 * hi >= 0)
...
def test_multi_car():
    car = Vehicle(0, 0, np.pi / 6)
    Xcar = copy(car)
    mpc_controller = MPCController(car, T=0.2, N=30)
    x_ref = np.array([[0, 0, np.pi / 6, 0, 0], [10, 4, np.pi / 6, 0, 0]])
    u_ref = np.zeros((mpc_controller.N, 2))

    car9 = Vehicle(10, 4, 2 * np.pi / 3)
    Xcar9 = copy(car9)
    mpc_controller9 = MPCController(car9, T=0.2, N=30)
    x_ref9 = np.array([[10, 4, 2 * np.pi / 3, 0, 0], [4, 10, 2 * np.pi / 3, 0, 0]])
    u_ref9 = np.zeros((mpc_controller9.N, 2))

    car11 = Vehicle(4, 10, -2* np.pi / 3)
    Xcar11 = copy(car11)
    mpc_controller11 = MPCController(car11, T=0.2, N=30)
    x_ref11 = np.array([[4, 10, -2* np.pi / 3, 0, 0], [0, 0, -2* np.pi / 3, 0, 0]])
    u_ref11 = np.zeros((mpc_controller11.N, 2))

    obs = np.array([[5, 2, 1.0], [7, 7, 1.0], [2, 6, 1.0]])
    ...
    line9, = ax.plot([], [], 'r:', linewidth=2)
    line11, = ax.plot([], [], 'r:', linewidth=2)

    # plot line for car 1 and 2
    line1, = ax.plot([], [], 'g:', linewidth=2)
    line2, = ax.plot([], [], 'y:', linewidth=2)
    line3, = ax.plot([], [], 'y:', linewidth=2)
    line4, = ax.plot([], [], 'y:', linewidth=2)
    ...
    ax.plot(x_ref[0, 0], x_ref[0, 1], 'g*')  # Green star for start
    ax.plot(x_ref[-1, 0], x_ref[-1, 1], 'ro')  # Red circle for goal
    ax.plot(x_ref9[0, 0], x_ref9[0, 1], 'g*')  # Green star for start
    ax.plot(x_ref9[-1, 0], x_ref9[-1, 1], 'ro')  # Red circle for goal
    ax.plot(x_ref11[0, 0], x_ref11[0, 1], 'g*')  # Green star for start
    ax.plot(x_ref11[-1, 0], x_ref11[-1, 1], 'ro')  # Red circle for goal

    # Plot current car position
    rect = plt.Rectangle((0, 0), 0.5, 1, angle=0, color='r', fill=True, label='Robot 1', alpha=0.5)
    ax.add_patch(rect)

    rect1 = plt.Rectangle((0, 0), 0.5, 1, angle=0, color='g', fill=True, label='dyna obs 1', alpha=0.2)
    ax.add_patch(rect1)

    rect2 = plt.Rectangle((0, 0), 0.5, 1, angle=0, color='g', fill=True, label='dyna obs 2', alpha=0.2)
    ax.add_patch(rect2)

    rect3 = plt.Rectangle((0, 0), 0.5, 1, angle=0, color='g', fill=True, label='dyna obs 3', alpha=0.2)
    ax.add_patch(rect3)

    rect4 = plt.Rectangle((0, 0), 0.5, 1, angle=0, color='g', fill=True, label='dyna obs 4', alpha=0.2)
    ax.add_patch(rect4)

    rect9 = plt.Rectangle((0, 0), 0.5, 1, angle=0, color='y', fill=True, label='Robot 2', alpha=0.5)
    ax.add_patch(rect9)

    rect11 = plt.Rectangle((0, 0), 0.5, 1, angle=0, color='b', fill=True, label='Robot 3', alpha=0.5)
    ax.add_patch(rect11)

    # add legend for robot, robot 9, robot 11
    ax.legend()

    car1 = Vehicle(2, 2, 0.1, 0.5, -0.0)
    car2 = Vehicle(5, 8, 0.1, -0.5, 0.0)
    car3 = Vehicle(9, 8, 0.1, -0.5, -0.0)
    ...
    with writer.saving(fig, "multi_car_animation.mp4", 600):
        while not mpc_controller.car.is_goal_reached(mpc_controller.car.path[-1], x_ref[-1]) or \
            ...
            if (distance([car1.x, car1.y], [car.x, car.y]) < 5):
                dynamic_car_trajectories1.append(car1.prediction_traj(mpc_controller.N, u_ref_car1, mpc_controller.T))
            if (distance([car2.x, car2.y], [car.x, car.y]) < 5):
                dynamic_car_trajectories1.append(car2.prediction_traj(mpc_controller.N, u_ref_car2, mpc_controller.T))
            if (distance([car3.x, car3.y], [car.x, car.y]) < 5):
                dynamic_car_trajectories1.append(car3.prediction_traj(mpc_controller.N, u_ref_car3, mpc_controller.T))
            if (distance([car4.x, car4.y], [car.x, car.y]) < 5):
                dynamic_car_trajectories1.append(car4.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))

            if (distance([Xcar9.x, Xcar9.y], [car.x, car.y]) < 5):
                dynamic_car_trajectories1.append(Xcar9.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))

            if (distance([Xcar11.x, Xcar11.y], [car.x, car.y]) < 5):
                dynamic_car_trajectories1.append(Xcar11.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))

            # create obs1 for car9 and obs
            obs1 = obs.copy()
            obs1 = np.append(obs1, [[Xcar9.x, Xcar9.y, 2.0]], axis=0)
            obs1 = np.append(obs1, [[Xcar11.x, Xcar11.y, 2.0]], axis=0)

            time_start = time.time()

            x_opt, u_opt = mpc_controller.planning(x_ref, u_ref, obs1, dynamic_car_trajectories1)

            end1 = time.time()
            # time in ms
            print("Time for car 1", 1000 * (end1 - time_start))
            ...
            if (distance([car1.x, car1.y], [car9.x, car9.y]) < 5):
                dynamic_car_trajectories2.append(car1.prediction_traj(mpc_controller.N, u_ref_car1, mpc_controller.T))
            if (distance([car2.x, car2.y], [car9.x, car9.y]) < 5):
                dynamic_car_trajectories2.append(car2.prediction_traj(mpc_controller.N, u_ref_car2, mpc_controller.T))
            if (distance([car3.x, car3.y], [car9.x, car9.y]) < 5):
                dynamic_car_trajectories2.append(car3.prediction_traj(mpc_controller.N, u_ref_car3, mpc_controller.T))
            if (distance([car4.x, car4.y], [car9.x, car9.y]) < 5):
                dynamic_car_trajectories2.append(car4.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))

            if (distance([Xcar.x, Xcar.y], [car9.x, car9.y]) < 5):
                dynamic_car_trajectories2.append(Xcar.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))

            if (distance([Xcar11.x, Xcar11.y], [car9.x, car9.y]) < 5):
                dynamic_car_trajectories2.append(Xcar11.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))

            obs2 = obs.copy()
            obs2 = np.append(obs2, [[Xcar.x, Xcar.y, 2.0]], axis=0)
            obs2 = np.append(obs2, [[Xcar11.x, Xcar11.y, 2.0]], axis=0)

            x_opt9, u_opt9 = mpc_controller9.planning(x_ref9, u_ref9, obs2, dynamic_car_trajectories2)
            a9, omega9 = u_opt9[0]

            # add car 11
            dynamic_car_trajectories3 = []
            if (distance([car1.x, car1.y], [car11.x, car11.y]) < 5):
                dynamic_car_trajectories3.append(car1.prediction_traj(mpc_controller.N, u_ref_car1, mpc_controller.T))
            if (distance([car2.x, car2.y], [car11.x, car11.y]) < 5):
                dynamic_car_trajectories3.append(car2.prediction_traj(mpc_controller.N, u_ref_car2, mpc_controller.T))
            if (distance([car3.x, car3.y], [car11.x, car11.y]) < 5):
                dynamic_car_trajectories3.append(car3.prediction_traj(mpc_controller.N, u_ref_car3, mpc_controller.T))
            if (distance([car4.x, car4.y], [car11.x, car11.y]) < 5):
                dynamic_car_trajectories3.append(car4.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))

            if (distance([Xcar.x, Xcar.y], [car11.x, car11.y]) < 5):
                dynamic_car_trajectories3.append(Xcar.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))
            if (distance([Xcar9.x, Xcar9.y], [car11.x, car11.y]) < 5):
                dynamic_car_trajectories3.append(Xcar9.prediction_traj(mpc_controller.N, u_ref_car4, mpc_controller.T))

            obs3 = obs.copy()
            obs3 = np.append(obs3, [[Xcar.x, Xcar.y, 2.0]], axis=0)
            obs3 = np.append(obs3, [[Xcar9.x, Xcar9.y, 2.0]], axis=0)

            x_opt11, u_opt11 = mpc_controller11.planning(x_ref11, u_ref11, obs3, dynamic_car_trajectories3)
            a11, omega11 = u_opt11[0]
            ...
            x_ref11[0, :] = [x11, y11, psi11, v11, delta11]
            u_ref11 = np.concatenate((mpc_controller11.u0, u_ref11[mpc_controller11.N:]))
            ...
            rect11.set_width(robot_length)
            rect11.set_height(robot_width)
            rect11.angle = np.degrees(psi11)
            # Compute the center of the rectangle with angle psi from traj1
            x11 = x_opt11[0, 0]
            y11 = x_opt11[0, 1]

            center_x11 = x11 - robot_length / 2 * np.cos(psi11) + robot_width / 2 * np.sin(psi11)
            center_y11 = y11 - robot_length / 2 * np.sin(psi11) - robot_width / 2 * np.cos(psi11)
            # Adjust to rear center
            rx11 = center_x11 + rear_axle_x * np.cos(psi11)
            ry11 = center_y11 + rear_axle_x * np.sin(psi11)
            rect11.set_xy((rx11, ry11))
